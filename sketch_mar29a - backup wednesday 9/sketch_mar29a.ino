...
// Fix the startupLightShow function
void startupLightShow() {
  Serial.println("Running LED startup animation");
  
  // Instead of directly calling rainbowCycle, use LED modes that are already defined
  setLightMode(LIGHT_MODE_ON);  // Turn on lights
  delay(500);                   // Wait a moment
  setLightMode(LIGHT_MODE_OFF); // Turn off lights
  delay(500);                   // Wait a moment
  setLightMode(LIGHT_MODE_ON);  // Turn on again
  
  // Return to default state based on mode
  updateLightBasedOnMode(50); // Use a mid-range value to start
}
...
void setup() {
  ...
  // Run our cool LED startup animation
  startupAnimation();
