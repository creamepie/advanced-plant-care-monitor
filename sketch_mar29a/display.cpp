...
void drawMainScreen() {
  ...
  // Add pump control button
  drawControlButton("PUMP", PUMP_BUTTON_X, PUMP_BUTTON_Y, PUMP_BUTTON_W, PUMP_BUTTON_H, BLUE, false);
...
void updateDisplay(int lightPercent, int moisturePercent, float temperature, float humidity, bool fanState) {
  ...
  if (lightPercent != lastLightPercent) {
    updateSensorValue("LIGHT", BOX_MARGIN, HEADER_HEIGHT + BOX_MARGIN, 
                      BOX_WIDTH, BOX_HEIGHT, lightPercent, LIGHT_COLOR);
    lastLightPercent = lightPercent;
  }

  if (moisturePercent != lastMoisturePercent) {
    updateSensorValue("MOISTURE", tft.width() - BOX_MARGIN - BOX_WIDTH, 
                      HEADER_HEIGHT + BOX_MARGIN, BOX_WIDTH, BOX_HEIGHT, 
                      moisturePercent, MOISTURE_COLOR);
    lastMoisturePercent = moisturePercent;
  }

  if (temperature != lastTemperature) {
    updateSensorValue("TEMPERATURE", BOX_MARGIN, 
                      HEADER_HEIGHT + BOX_MARGIN*2 + BOX_HEIGHT, 
                      BOX_WIDTH, BOX_HEIGHT, temperature, TEMP_COLOR);
    lastTemperature = temperature;
  }

  if (humidity != lastHumidity) {
    updateSensorValue("HUMIDITY", tft.width() - BOX_MARGIN - BOX_WIDTH, 
                      HEADER_HEIGHT + BOX_MARGIN*2 + BOX_HEIGHT, 
                      BOX_WIDTH, BOX_HEIGHT, humidity, HUMIDITY_COLOR);
...
void updateSensorValue(String type, int x, int y, int w, int h, float value, uint16_t color) {
  // Clear previous value - make this area larger to ensure old text is fully erased
  tft.fillRect(x + 10, y + 25, w - 20, 40, CARD_COLOR);
  ...
  // Use consistent text size - was missing before
  tft.setTextSize(3);
...
void refreshDisplay(int lightPercent, int moisturePercent, float temperature, float humidity, bool fanState) {
  ...
  if (currentTime - lastRefreshTime >= REFRESH_INTERVAL) {
    ...
    updateDisplay(lightPercent, moisturePercent, temperature, humidity, fanState);
...
void handleTouchInput() {
  // Stub function to prevent undefined reference
  // Implement real touch handling here if needed
}

void handleTouchInputWithTimeout() {
  // Get touch point
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    // Check if touch is on the fan button
    if (x > FAN_BUTTON_X && x < FAN_BUTTON_X + FAN_BUTTON_W &&
        y > FAN_BUTTON_Y && y < FAN_BUTTON_Y + FAN_BUTTON_H) {
      uint8_t currentFanMode = getFanMode();
      uint8_t newFanMode = (currentFanMode + 1) % 3; // Cycle through OFF, ON, AUTO
      setFanMode(newFanMode);
      updateFanStatus(getFanState());
    }

    // Check if touch is on the pump button
    if (x > PUMP_BUTTON_X && x < PUMP_BUTTON_X + PUMP_BUTTON_W &&
        y > PUMP_BUTTON_Y && y < PUMP_BUTTON_Y + PUMP_BUTTON_H) {
      uint8_t currentPumpMode = getPumpMode();
      uint8_t newPumpMode = (currentPumpMode + 1) % 3; // Cycle through OFF, ON, AUTO
